...
public class GerenciadorComunidade {
	static CommunityDAO temp = new CommunityDAO();
	...
	public static void createCommunity(int idCreator){
		...
		Community auxCommunity = new Community( idCreator,title,description);
	...
	public static List<Community> communitiesByIdUser(int idUser){
		return temp.returnCommunityListOfUser(idUser);
	}
	
	public static ArrayList<Community> searchOthersCommunities(int idUser){
		ArrayList<Community> others = new ArrayList<Community>();
		List<Community> myCommunities = communitiesByIdUser(idUser);
		boolean aux;
	...
	public static void myCommunities(int idUser){
		List<Community> my = communitiesByIdUser(idUser);
		int option;
	...
	public static Community returnCommunity(int idCommunity){
		return temp.retornar(idCommunity, Community.class);
	}
	...
	public static void addArticle(int index,int idUser){
		Community community = temp.retornar(index, Community.class);
		community.CreateArticle(idUser);
		temp.salvar(community);
	}
	
	public static void addTopic(int index, int idUser){
		Community community = temp.retornar(index, Community.class);
		community.CreateTopic(idUser);
		temp.salvar(community);
	}
	
	public static void addQuestion(int index, int idUser){
		Community community = temp.retornar(index, Community.class);
		community.CreateQuestion(idUser);
		temp.salvar(community);
	}
